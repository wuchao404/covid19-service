...
public class SnowflakeUtils {
    ...
    public synchronized static long genId(){
        ...
        //上次生成ID的时间截
        LAST_TIME_STAMP = now;
        long first = ((now - START_TIME) << TIME_LEFT_BIT);
        long second = (DATA_ID << DATA_LEFT_BIT);
        long third = (WORK_ID << WORK_LEFT_BIT) | LAST_SEQ;
    ...
    public static void main(String[] args) {
        ...
        String hostName = "bongo";
        byte[] bytes = hostName.getBytes();
        int sums = 0;
        for(int b : bytes){
            sums += b;
        }
        System.out.println("sums:" + bytes[0]);
